...
from getOID.Nodo import Nodo
from getOID.ComboBoxWindow import ComboBoxWindow
...
Hijo = raiz.hijos[0]
tipos = [["sysDescr", iso + ".1.1.0", "Describe el sistema", "0"], ["sysObjectID", iso + ".1.2.0", "id", "0"],
         ["sysContact", iso + ".1.4.0", "Contacto", "0"]]
# [1, "Interfaces", iso + ".2" ] ]
for x in tipos:
    Hijo.addChildren(Nodo(x[0], x[1], x[2], x[3]))

Hijo = raiz.hijos[1]
tipos = [["ifNumber", iso + ".2.1.0", "Da el numero de interfaces", "0"],
         ["ifTable", "1.3.6.1.2.1.2.2", "Tabla de interfaces", "2"]]
# [1, "Interfaces", iso + ".2" ] ]
for x in tipos:
    Hijo.addChildren(Nodo(x[0], x[1], x[2], x[3]))
...
tipos = [["icmpInMsgs", iso + ".5.1.0", "Da el numero de mensajes icmp entrantes", "1"],
         ...
         ["icmpInDestUnreachs", iso + ".5.3.0", "Da el numero de mensajes icmp entrantes con destino inalcanzable",
          "1"],
         ...
         ["icmpOutDestUnreachs", iso + ".5.3.0", "Da el numero de mensajes icmp salientes con destino inalcanzable",
          "1"],
